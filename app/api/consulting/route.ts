...
import { getResend, getSenderEmail, getNotificationEmail, ConsultationBookingData } from '@/lib/resend'
...
export async function POST(req: Request) {
  try {
    ...
    if (resend) {
      // 1. Send notification email to the business / owner (e.g. ahsan.yousaf.dev@gmail.com / nspears@nbsfreightllc.com)
      try {
      ...
      try {
        ...
        if (clientResult.error) {
          ...
          // Note: On free/unverified Resend accounts, sending to external addresses fails if domain is unverified
